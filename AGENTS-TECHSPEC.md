...
- If the diagnostic Codex request fails, the bot shows the available Codex error detail from raw output instead of only a generic exit code.
...
- Agent-facing documentation starts from `AGENTS.md`; `CLAUDE.md` is a compatibility entrypoint that points back to the same workflow.
- The functional specification lives in `AGENTS-TECHSPEC.md`; `FS.md` is a compatibility pointer only.
- Before code or behavior changes, update `AGENTS-TECHSPEC.md` when the requested task changes the functional contract.
- Keep `AGENTS-HISTORY.md` as a concise changelog of agent work and `AGENTS-SUMMARY.md` as the current handoff snapshot for clean-context agents.
- The repository includes a short technical note `AGENTS-SPECIFICS-1.md` that explains how the bot talks to Codex CLI and how CLI output is streamed or finalized back into Telegram chat.
