...
- appends the full command and raw Codex output to `history/<TelegramUserID>/<project-name>.log`;
...
## Project And Session Diagnostics

When a user explicitly selects a project or session, the bot runs a short Codex diagnostic request before showing the final selection card.

- Project selection uses the latest saved session for that project when one exists.
- Session selection uses `codex exec resume` for the selected `codex_session_id`.
- The diagnostic card includes only metadata that Codex actually returns. Context is shown only as remaining percentage when the context window and token usage are both available. Five-hour and weekly limits are hidden when Codex CLI does not report them.
- If the diagnostic run fails, the card includes available raw Codex error detail instead of only a generic exit code.
