...
- При смене проекта следующий запуск начинается в новой сессии.
...
- Везде, где бот показывает идентификатор сессии пользователю, используется формат `project|tail|YY-MM-DD|HH:MM`, где `tail` — финальный сегмент `codex_session_id` вместе с ведущим `-`, а дата и время берутся из `updated_at` последнего использования сессии.
