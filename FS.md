...
- Верхняя строка статуса рендерится жирным шрифтом; вместо произвольного цвета текста используется синий эмодзи-индикатор, так как Telegram Bot API не поддерживает свободный выбор цвета текста.
