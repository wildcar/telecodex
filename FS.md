...
- Хранилище: SQLite.
- Стартовый список проектов задается через `TELECODEX_PROJECTS_JSON`, формат `{"demo":"/opt/projects/demo","infra":"/opt/projects/infra"}`; проекты, добавленные из Telegram UI, сохраняются в SQLite и доступны после рестарта.
...
- Стартовый список проектов из `TELECODEX_PROJECTS_JSON` импортируется в БД только один раз при первом запуске; дальше источником истины для списка проектов является SQLite, чтобы удаление и добавление проектов сохранялось после рестарта.
