...
8. Для каждого Telegram user id дополнительно ведется plaintext-файл `HISTORY_DIR/coversation<TelegramUserID>.log` с timestamp, сообщением пользователя, полной командой запуска и сырым ответом Codex.
...
- Пользовательские conversation-логи: `HISTORY_DIR/coversation*.log`.
